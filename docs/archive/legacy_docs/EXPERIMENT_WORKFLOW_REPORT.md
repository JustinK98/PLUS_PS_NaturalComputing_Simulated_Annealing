...
Normal suite runs are validation-only. Fixed-profile evaluation additionally
writes `test_loss_by_layout.png` and test metrics into JSON and CSV artifacts.
...
Per concrete Online-Delta run, the learning-rate folder additionally contains:

```text
plots_single/<run_id>_layout_summary.png
plots_single/<run_id>_accepted_timeline.png
```

These compact plots make the random start layout, the accepted `set_neuron`
changes, and the best/end layouts inspectable without opening the GUI.

Optional per-step PNG frames are generated only with:

```bash
python main.py experiment suite --exp online-delta --benchmark two_moons --export-layout-frames
```

The explicit export adds:

```text
layout_frames/<run_id>/frame_000_start.png
layout_frames/<run_id>/frame_<n>_step_<step>.png
layout_frames/<run_id>/accepted_changes.csv
```

Frame series are regenerable visual artifacts and are not part of the compact
default evaluation output.

## 11. Confirmed Tuned Result Snapshot

The fixed evaluation profile `tuned_20260530` comes from:

```text
outputs/hyperparameter_tuning/20260530_143043_overnight
```

The table summarizes the final 30-run confirmation. Lower validation loss is better.

| Benchmark | Online best retrained | Matched random | Paired improvement | Win rate | Best homogeneous | Acceptance |
| --- | ---: | ---: | ---: | ---: | --- | ---: |
| `two_moons` | `0.08893` | `0.09072` | `+0.00178` | `0.533` | `all_gelu` (`0.08391`) | `0.506` |
| `concentric_circles` | `0.02953` | `0.02961` | `+0.00008` | `0.533` | `all_swish` (`0.02663`) | `0.306` |
| `crossing_spirals` | `0.70738` | `0.66226` | `-0.04512` | `0.433` | `all_relu` (`0.63821`) | `0.679` |

Interpretation:

- `two_moons`: healthy Online-Delta numerics and a small paired improvement.
- `concentric_circles`: healthy numerics but effectively neutral SA effect on an already saturated benchmark.
- `crossing_spirals`: ordinary tuned SGD learns the benchmark, but Online-Delta degrades the matched random baseline.

The accurate statement is:

> Online-Delta-SA is not a universal improvement mechanism under the evaluated setup. It produces a limited positive signal on `two_moons`, is effectively neutral on `concentric_circles`, and is negative on `crossing_spirals`.
...
| `configs/evaluation_profiles.json` | Promoted benchmark-specific evaluation parameters |
...
> The Online-Delta-SA suite evaluates whether local activation-function changes can find mixed layouts that improve over matched random starts. After benchmark-specific validation-only tuning and 30-run confirmation, the effect is small on `two_moons`, neutral on `concentric_circles`, and negative on `crossing_spirals`. The results do not support a general claim that Online-Delta-SA improves activation layouts.
